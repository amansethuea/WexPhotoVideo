...
class IssueTypePrediction(object):
    def __init__(self):
        if sys.platform.startswith("win"):
            ...
            self.df = pd.read_csv(self.path + 'final_bert_predictions.csv') # Data with predictions
    ...
    def reviews_sentiment_distribution(self, stream_lit=False, dash=False):
      ...
      for i, source in enumerate(['Trustpilot', 'PowerReviews']):
          source_data = filtered_df[filtered_df['Source'] == source]['Predictions'].value_counts().reset_index()
          source_data.columns = ['Predictions', 'Count']
        
          fig.add_trace(
              go.Pie(
                  labels=source_data['Predictions'],
    ...
    def pre_requisite_before_visuals(self):
        # Fresh df read
        self.df = pd.read_csv(self.path + 'final_bert_predictions.csv')
        print("START: Initiating predicting Issue Type")
        filtered_df = self.df[self.df['Predictions'].isin(['Positive', 'Negative', 'Neutral'])]
        ...
        print("Issue Types predicted successfully. Please check bert_predictions_with_issue_types.csv")
